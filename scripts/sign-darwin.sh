...
# self-signed cert reused from desk-agent's signing setup.
#
...
#
# Cert setup is one-time in ~/Documents/dev/desk-agent/scripts/
...
if ! security find-certificate -c "$IDENTITY" >/dev/null 2>&1; then
  ...
  echo "         (run ~/Documents/dev/desk-agent/scripts/create-signing-cert.sh first if you want stable signatures)" >&2
  exit 0
fi
