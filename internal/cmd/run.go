...
Run options:
  ...
  --model <id>            Model: alias (glm|gemini|deepseek|kimi|sonnet|opus), provider/model,
                          or bare model. Default: $MM_RUN_MODEL or gemini/gemini-3.6-flash.
...
mm run list and mm run show read those reports from the CLI.`,
	...
	cmd.Flags().StringP("model", "m", "", "Model: alias (glm|gemini|deepseek|kimi|sonnet|opus), provider/model, or bare model. Default: $MM_RUN_MODEL or gemini/gemini-3.6-flash")
